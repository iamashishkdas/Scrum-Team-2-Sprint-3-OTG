...
import com.t2s3.pages.T2S3_OAP1797_Page;
...
public class T2S3_OAP1797
{
	public WebDriver driver;
	ChromeOptions options;
	GenericFunctionsOAP gf;
	T2S3_OAP1797_Page obj;
	Utility util;
	Properties prop;
	...
	public void stageApplicationSetUp() {
		try {
			...
			obj = new T2S3_OAP1797_Page(driver);
	...
	public void TC03_NavigateToNewQuestionBankPage() throws IOException, InterruptedException {
		try {
			...
			i=obj.navigateToNewQuestionBankPage();
			 Assert.assertEquals(i, 1);
				test.pass("Navigated to New Question Bank Page Validated Successfully");
				takeScreenshot();
		} catch (WebDriverException | AssertionError e) {
			test.fail("Navigation to New Question Bank Page Validation unsuccessful");
			 takeScreenshot();
			    test.fail(e);
			    throw new AssertionError("Test failed: " + e.getMessage());
		}
	...
	public void TC04_NavigateToAddMCQPage() throws IOException, InterruptedException {
		try {
			...
			i=obj.toAddMCQPage();
			Assert.assertEquals(i, 1);
			test.pass("Navigated to add MCQ page Successfully");
			takeScreenshot();
		} catch (WebDriverException | AssertionError e) {
			test.fail("Navigation to add MCQ page unsuccessful");
			takeScreenshot();
			test.fail(e);
			throw new AssertionError("Test failed: " + e.getMessage());
		}
	...
	@Description("TC05_Validate add Alternative Question disabled")
	@Step("Validate Publish Button of MCQ")
	@Test(priority = 5, testName = "TC05_Validate add Alternative Question disabled")
	public void TC05_ValidateAddAlternativeQuestionDisabled() throws IOException, InterruptedException {
		try {
			test.info("Test to validate Buttons");
			i=obj.addAlternativeQuestionDisabled();
			Assert.assertEquals(i, 1);
			test.pass("Alternative Question button disabled and Publish button Enabled after adding required fields Validated Successfully");
			takeScreenshot();
		} catch (WebDriverException | AssertionError e) {
			test.fail("Alternative Question button disabled and Publish button Enabled after adding required fields Validation unuccessfully");
			takeScreenshot();
			test.fail(e);
			throw new AssertionError("Test failed: " + e.getMessage());
		}
	...
	@Description("TC08_Validate 2 Alternative Questions")
	@Step("Validate 2 Alternative Questions")
	@Test(priority = 8, testName = "TC08_Validate 2 Alternative Questions")
	public void TC08_Validate2AlternativeQuestions() throws IOException, InterruptedException {
		try {
			test.info("Test to validate 2 Alternative Questions");
			i=obj.alternativeQuestions2();
			Assert.assertEquals(i, 1);
			test.pass("2 Alternative Questions present & button disabled validated Successfully");
			takeScreenshot();
		} catch (WebDriverException | AssertionError e) {
			test.fail("2 Alternative Questions present & button disabled validated unsuccessful");
			takeScreenshot();
			test.fail(e);
			throw new AssertionError("Test failed: " + e.getMessage());
		}
